...
function ViewOrderNescafe() {
    const [info, setInfo] = useState([]);
    useEffect(() => {
        axios.get('https://no-queue-webapp.herokuapp.com/Nescafe/orders')
  .then((response) => {
    ...
    return (
        <div style={{backgroundColor: "#FEFBF3",backgroundImage: `url(${"https://www.transparenttextures.com/patterns/brick-wall.png"})`,minHeight:"100vh"}}>
            ...
            {info.map((note, index)=>{
                return <NoteOrder items={note.Order.Items} qt={note.Order.Qt} total={note.Order.Total} name={note.Name} phone={note.Phone} shopName="Nescafe"/>;
            })}
...
export default ViewOrderNescafe;
